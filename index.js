...
app.post("/recieve", (req, res) => {
	...
	if (req.body.event.type == "reaction_added") {
		...
		Users.findOne({ "uuid": req.body.event.user }, (err, user) => {
			...
			if (user) {
				...
				if (!one.reactions) {
					...
					one.reactions.total = 0

				}

				if (one.reactions[req.body.event.reaction]) {

					one.reactions[req.body.event.reaction] += 1;

					one.reactions.total += 1
				...
				Users.findOneAndUpdate({ "uuid": one.uuid }, { "reactions": one.reactions }, (err, doc) => console.log(doc));

			} else {

				one = new Users()

				request.post({ url: 'https://slack.com/api/users.info', form: { token: process.env.TOKEN, user: req.body.event.user } }, function (err, httpResponse, body) {

					if (JSON.parse(body).user) {

						one.name = JSON.parse(body).user.real_name

						one.uuid = req.body.event.user
		...
		console.log("I HAVE BEEN ACTIVEATE--------")

		Channels.findOne({ "uuid": req.body.event.item.channel }, (err, user) => {
			...
			if (user) {
				...
				Channels.findOneAndUpdate({ "uuid": one.uuid }, { "reactions": one.reactions }, (err, doc) => console.log(doc));

			} else {

				console.log("inside channels new one")

				one = new Channels()

				request.post({ url: 'https://slack.com/api/channels.info', form: { token: process.env.TOKEN, channel: req.body.event.item.channel } }, function (err, httpResponse, body) {

					console.log(body, req.body.event)

					if (JSON.parse(body).channel) {

						one.name = JSON.parse(body).channel.name

						one.uuid = req.body.event.item.channel
		...
		request.post({ url: 'https://slack.com/api/emoji.list', form: { token: process.env.TOKEN } }, function (err, httpResponse, body) {

			all = Object.keys(JSON.parse(body).emoji)

			all = all.concat(aemojis)
			...
			do {
				...
				text = String(text).match(regex)

				if (text) {

					emojis.push(text[1]);

					text = text["input"].replace(text[0], "");
			...
			Users.findOne({ "uuid": req.body.event.user }, (err, user) => {
				...
				if (user) {
					...
					if (!user.reactions) {
						...
						user.reactions.total = 0

					}

					final.map((val) => {

						if (user.reactions[val]) {

							user.reactions[val] += 1

							user.reactions.total += 1
					...
					Users.findOneAndUpdate({ "uuid": user.uuid }, { "reactions": user.reactions }, (err, doc) => console.log(doc));
					...
					request.post({ url: 'https://slack.com/api/users.info', form: { token: process.env.TOKEN, user: req.body.event.user } }, function (err, httpResponse, body) {
						...
						newuser.uuid = req.body.event.user

						newuser.reactions = {}

						newuser.reactions.total = 0

						if (JSON.parse(body).user) {

							newuser.name = JSON.parse(body).user.real_name

							final.map((val) => {

								if (newuser.reactions[val]) {

									newuser.reactions[val] += 1

									newuser.reactions.total += 1
			...
			Channels.findOne({ "uuid": req.body.event.channel }, (err, user) => {
				...
				if (user) {
					...
					Channels.findOneAndUpdate({ "uuid": user.uuid }, { "reactions": user.reactions }, (err, doc) => console.log(doc));

				} else {

					console.log("the gods have been triggered")

					newuser = new Channels()

					request.post({ url: 'https://slack.com/api/channels.info', form: { token: process.env.TOKEN, channel: req.body.event.channel } }, function (err, httpResponse, body) {

						newuser.uuid = req.body.event.channel
						...
						if (JSON.parse(body).channel) {
							...
							newuser.name = JSON.parse(body).channel.name
...
app.get("/", (req, res) => {
	...
	res.send("IM AWAKE AND WANT TO GO TO SLEEP")
...
app.get("/data", (req, res) => {
	...
	Users.find({}, (err, doc) => {
		...
		res.send(sort(doc).desc(u => u.reactions.total))
...
app.get("/custompng", (req, res) => {
	res.setHeader("Cache-Control", "max-age=600")
	request.post({ url: 'https://slack.com/api/emoji.list', form: { token: process.env.TOKEN } }, function (err, httpResponse, body) {
		res.send(JSON.parse(body).emoji);

	})

})

app.listen(3000, (err) => console.log("Emoji Thingy Up and Running"))
